...
/*   Updated: 2019/07/22 06:31:40 by mabayle          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minishell.h"

char	*replace_norme(char *input, int i, char *cmd, char *new)
{
	if ((i == 0 || (i != 0 && ft_isspace(input[i - 1])))
		&& input[i] == '~' && input[i + 1] != '$')
	{
		if (input[i] == '~')
...
char	*replace(char *input, char *cmd, int i)
{
	...
	while (input[i])
	{
		if (input[i] == '$' && ft_strlen(input) > 1)
		{
			...
			new = replace_norme(input, i, cmd, new);
		i++;
	}
	free(input);
	return (new);
